...
def test_human_signoff_cannot_auto_approve() -> None:
    ...
    # The checked-in artifact may be either the pre-review checkpoint or the
    # later human-approved closure.  In both states, approval must be explicit
    # and the guard against automatic signing must remain enabled.
    assert signoff["no_auto_sign"] is True
    if signoff["status"] == "WAITING_GATE28B_HUMAN_REVIEW":
