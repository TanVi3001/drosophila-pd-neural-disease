...
def test_action_hook_contract_exists_and_has_discovery_status() -> None:
    ...
    assert document["schema_version"] == "gate-12f-a-action-hook-contract-v2"
    ...
    assert "healthy_baseline.py" in action_pipeline["action_source_file"]
    assert "perturbation.apply_to_action" in action_pipeline["action_hook_symbol"]
...
def test_discovery_manifest_preserves_no_execution_boundary() -> None:
    ...
    assert manifest["integration_mode"] == "platform_perturbation_protocol"
