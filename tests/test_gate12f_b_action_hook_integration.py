...
def test_gate12f_b_artifacts_preserve_platform_boundary() -> None:
    ...
    assert config["platform"]["source_patch_required"] is False
    assert manifest["platform_source_patch_required"] is False
    assert manifest["source_patch_applied"] is False
    ...
    assert manifest["status"] == "CONNECTED_TO_PLATFORM_PERTURBATION_PROTOCOL"
    assert "No source patch is required" in report
