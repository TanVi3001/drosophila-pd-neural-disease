...
def _joint_angles(action: Any, expected_joint_angle_count: int | None = 42) -> np.ndarray:
    ...
    expected_shape = (
        JOINT_ANGLES_SHAPE
        if expected_joint_angle_count is None
        else (int(expected_joint_angle_count),)
    )
...
def apply_proxy_operator_to_locomotion_action(
    ...
    *,
    ...
    seed: int | None = None,
    expected_joint_angle_count: int | None = 42,
) -> Any:
    ...
    source = _joint_angles(locomotion_action, expected_joint_angle_count)
